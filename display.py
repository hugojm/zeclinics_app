    ...
    with torch.set_grad_enabled(False):
        ...
        for i,im in enumerate(image_names):
            for j,feno in enumerate(feno_names):
                fenos[im][feno] = int(out[i][j])
    return fenos

def generate_and_save_predictions(plate_path,batch_size,model_path_seg,model_path_bools,mask_names,feno_names,device):
