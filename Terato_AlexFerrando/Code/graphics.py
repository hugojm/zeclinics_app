...
'''
Each functions returns a plot, except the function plot_coordinates taht is a subfunction od¡f the mca
The input is always the same:
    df: a dataframe with the predictions, we will use the stats.csv converted in dataframe
    path: where we want to save the plots
'''

'''
Given a dataframe with the predictions we plot two histograms,
one with the death probability in function if the dose for the plate compound
and another with the illness probability for each phenotype.

This plots are save as a html file since they are interactive
'''
def doseresponse(df, path):
  ...
  '''
  In the illness probability graphic we plot a black line to indicate the probability with the DMSO compound
  '''
  ...
  for c in compounds:
    ...
    graphic.save(path + '/' + c + '.html')


'''
Given the MCA coefficients and the input dataframe, returns an scatter plot with the column coordinates
'''
...
'''
Calculates the MCA coefficients of the predicted phenotypes and returns the mca plot
'''
def Mca(df, path):
    ...
    ax.get_figure().savefig(path + '/mca.png')


'''
Given a dataframe with the predictions, returns one scatterplot area vs dose for each mask and phenotype
indicating the observations that has presented the specific phenotype, In total, it saves 9 .png files
with 9 scatterplots each file.
'''
